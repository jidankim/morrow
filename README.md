...
7. Phase 7 wider-product rollout is covered by local final-smoke artifacts under `.omo/evidence/phase-7-wider-product-rollout/final-smoke`. It proves the static local rollout manifest, docs/privacy/runbook gates, retention QA, regression gate, kill-switch checks, privacy inspection, negative fixtures, artifact freshness, and cleanup. The current final-smoke receipt records `manifest_state: beta`, `default_availability_allowed: false`, `beta_readiness: BLOCKED`, `regression_gate: BLOCKED`, and `deployment_action=none`. This is not default availability, not live deployment, not live remote config, not telemetry upload, not provider network use, and not automatic promotion. The final release handoff ledger is `docs/wider-product-rollout-release-checklist.md`.
...
Public privacy and rollout claim boundaries live in `docs/privacy.md`, `docs/wider-product-rollout.md`, and `docs/wider-product-rollout-release-checklist.md`. The current rollout state remains beta/blocked with default availability false and no deployment action.
