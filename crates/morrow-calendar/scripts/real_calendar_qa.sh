...
if [[ "$(uname -s)" != "Darwin" ]]; then
  printf 'BLOCKED: real Calendar QA requires macOS Calendar/EventKit\n' >&2
  exit 64
fi

script_dir="${0:A:h}"
objc_source="${script_dir}/real_calendar_qa.m"
command_display="crates/morrow-calendar/scripts/real_calendar_qa.sh"
binary_path="${TMPDIR:-/tmp}/morrow-calendar-real-qa-$$"
