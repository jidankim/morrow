...
Phase 0 is not full AXI compliance. It is the evidence baseline that now has Phase 1 production trace sink smoke coverage and Phase 2 product correlation smoke coverage, while Phase 3 lifecycle coverage, Phase 4 human approval/correction loop, Phase 5 trajectory eval hardening, and cloud telemetry rollout remain future work.
...
| strict privacy boundaries | The local smokes avoid live vendor backends and real Messages data. Diagnostics exports are sanitized, privacy inspection rejects canary leakage, and feedback snapshots use bounded excerpts plus metadata rather than full private content. Phase 1 production trace sink smoke uses the fake Codex production fixture and rejects an injected raw canary before sanitization. Phase 2 keeps correlation local-only. No raw content leaves the device. No raw prompt text, raw source content, provider JSON, native identifiers, or app-data paths are exposed. | Cloud telemetry rollout remains future work. |
