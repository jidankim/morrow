...
fn sync_scheduler_state_round_trips_through_temp_store_path() -> Result<(), String> {
    ...
    let scheduled = SyncSchedulerStateCommand {
        ...
        next_run_at: Some(1_783_000_420),
...
#[test]
fn sync_scheduler_state_rejects_too_short_interval() -> Result<(), String> {
    // Given
    let dir = tempfile::tempdir().map_err(|error| error.to_string())?;
    let store_path = dir.path().join("morrow.sqlite");
    let state = NativeBridgeState::default();
    let invalid = SyncSchedulerStateCommand {
        ...
        last_result: None,
        retry_attempt: 0,
        last_reason: Some("Invalid interval.".to_owned()),
        updated_at: 1_783_000_030,
    };
    ...
    // Then
    match error {
        Ok(_) => Err("too short interval_seconds should fail".to_owned()),
        Err(message) => {
            assert!(message.contains("interval_seconds"));
            Ok(())
        }
    }
}
